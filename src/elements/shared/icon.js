...
export class IconElement extends LitElement {
  ...
  static faNormalizeIconSpec(iconSpec, defaultPrefix = 'fas') {
    ...
    if (typeof iconSpec === 'undefined' || iconSpec === null) {
      val = null;
    } else if (IconElement.isIconLookup(iconSpec)) {
      val = iconSpec;
    } else if (Array.isArray(iconSpec) && iconSpec.length === 2) {
      val = { prefix: iconSpec[0], iconName: iconSpec[1] };
    } else if (typeof iconSpec === 'string') {
      val = { prefix: defaultPrefix, iconName: iconSpec };
    }
    return val;
  }
  ...
  loadIcon() {
    ...
    if (!this.iconLookup) {
      ...
      this.svgData = parser.parseFromString(
        `<span>${this.iconLookup.html.join('\n')}</span>`,
        'text/html'
